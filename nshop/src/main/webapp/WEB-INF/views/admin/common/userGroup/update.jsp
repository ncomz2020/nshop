...
<script>
    ...
    function updateAction() {
		if ($("#update_usr_grp_nm").val() == "") {
			swal({
		        title: '<spring:message code="label.usergroup.valid.input.name"/>'},function() {
					$("#update_usr_grp_nm").focus();
			});
			return;
		}
		$("#update_user_group_auth_string").val(getUserGroupAuthString());
	...
	function initTree() {
		var param = new Object();
		param.usr_grp_id = "${userGroup.usr_grp_id}";
		$("#tree").dynatree({
		...
		<div class="rowBox">
			<div class="unitBox" style="">
				<form id="updateForm" action="javascript:updateAction();">
					<input type="hidden" name="usr_grp_id" value="${userGroup.usr_grp_id}">
					<input type="hidden" id="update_user_group_auth_string" name="user_group_auth_string">
					<table class="amb_form_table lineAll">
						...
						<tbody>
							<tr>
								...
								<td>
									<input type="text" class="inp" style="width: 100%;" id="update_usr_grp_nm" name="usr_grp_nm" placeholder='<spring:message code="label.usergroup.name"/>' maxlength="25" autofocus value="<c:out value="${userGroup.usr_grp_nm}"/>" onfocus="this.value = this.value;">
								</td>
							</tr>
							<tr>
								<th class="center"><spring:message code="label.usergroup.description"/></th>
								<td>
									<input type="text" class="inp" style="width: 100%;" id="update_expln" name="expln" placeholder='<spring:message code="label.usergroup.description"/>' maxlength="50" value="<c:out value="${userGroup.expln}"/>" onfocus="this.value = this.value;">
								</td>
							</tr>
						</tbody>
					</table>
