...
function goListPage() {
	if(pageType == 'cancelList'){
		//취소교환반품 페이지 이동
		movePage('/admin/order/cancelList/list.ajax');
	}else{
		//주문 페이지 이동
	...
	swal({
		...
		success : function(data) {
			...
			if (result == "succ") {
				...
				movePage('detail.ajax', {order_seq: '${orderInfo[0].order_seq}',pageType:'orderList'});
			} else {
				swal({
			       	title: result, 
		      		type: "error",
...
<body>		
	...
	<div class="g_column w_1_1">
						...
						<tbody>
							...
							<c:if test="${userGrup eq 2}">
								<tr>
									...
									<td>
										<textarea id="memo" rows="3" cols="50" style="width: 1200px;" placeholder='<spring:message code="label.order.memo.placeholder"/>'>${orderInfo[0].store_id_memo}</textarea>
		...
		<div class="fr btnArea middle">
				<a href="javascript:goListPage();" class="amb_btnstyle blue middle">
				<spring:message code="label.common.list" /></a>
		</div>
