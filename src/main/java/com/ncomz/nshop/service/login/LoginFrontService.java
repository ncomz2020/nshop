...
public class LoginFrontService {
	 ...
	 * @return
	 */
	public String loginfront(String textId, String textNm, boolean bForce, HttpServletRequest request){
		...
		if(loginMapper.countPresenceId(textId)>0){
			...
			if (sessionUser != null) { //로그인 성공
				...
				System.out.println("AAAAAAAAAAAAAAAAAAABBBBBBBBBBB");
				//최고 상점 or 그냥 상점
				if(("3").equals(sessionUser.getUsr_grp_id())){
					resultMsg = "GO_MAIN";
				}
		...
		System.out.println("AAAAAAVVVVVVV : "+resultMsg);
	...
	private SessionUser getSessionUser(String usrId, String password, String loginGatewayIp) {
		SessionUser sessionUser = loginMapper.loginfront(usrId, SHA256Util.getEncrypt(password));
