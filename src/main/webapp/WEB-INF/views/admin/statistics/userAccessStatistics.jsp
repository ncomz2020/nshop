...
<script type="text/javascript">
				...
				success : function(data) {
					...
					if(ssData != null){
						for(var i=0; i<ssData.length; i++){
							inputStr += "<tr><td class='center'>"+ssData[i].access_date+"</td>";
							inputStr += "<td class='right'>"+ comma(ssData[i].count_user_id) +"</td>";
						}
						dateArray = ssData[0].date_array;
						amtArray = ssData[0].amt_array;
						chartMake(dateArray,amtArray);
					}else{
						inputStr = "<tr><td colspan='2'>조회된 데이터가 없습니다</td></tr>";
	...
	function goExcel() {
		var param = checkparam();
		$.download('accessExportAction.ajax', param, 'post');
	}
	
	function chartMake(dateArray,amtArray) {
		var lineData = {
			labels: dateArray,
			datasets: [{
				label: "방문자수",
				...
				borderColor : "rgba(215,75,61,1)",
				lineTension: 0.0,
				pointBackgroundColor:"rgba(215,141,231,0.5)",
...
<div class="rowBox mgT30">
	...
	<div class="g_column w_2_1 mgT20" style="width: 80%">
		<h3><span class="title">방문자분석</span></h3>
