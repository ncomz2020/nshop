...
<script type="text/javascript">
	...
	function search(){
		...
		if(param){
			$.ajax({
				url:"getSalesStatisticsListAction.ajax",
				type:'POST',
				data : param,
				dataType: 'json',
				success : function(data) {
					...
					if(ssData != null){
						for(var i=0; i<ssData.length; i++){
							inputStr += "<tr><td class='center'>"+ssData[i].pay_fin_datetime+"</td>";
							inputStr += "<td class='right'>"+ comma(ssData[i].payment_amt) +"</td>";
							inputStr += "<td class='right'>"+ comma(ssData[i].calcul_amt) +"</td>";
						}
						dateArray = ssData[0].date_array;
						calArray = ssData[0].cal_array;
						payArray = ssData[0].pay_array;
						chartMake(dateArray,calArray,payArray);
					}else{
						inputStr = "<tr><td colspan='3'>조회된 데이터가 없습니다</td></tr>";
	...
	function goExcel() {
		var param = checkparam();
		$.download('salesExportAction.ajax', param, 'post');
	}
	
	function chartMake(dateArray,calArray,payArray) {
		var lineData = {
			labels: dateArray,
			datasets: [{
				label: "매출액",
...
<div class="rowBox mgT30">
	...
	<div class="g_column w_1_1 mgT20">
		<h3><span class="title">매출분석</span></h3>
