...
<script type="text/javascript">
	...
	function init() {
		...
		var file_id = "${productInfo.file_id}";
		var category_id = "${productInfo.category_id}";

		var file_list = file_id.split(',');
		var category_list = category_id.split(',');

		var i, j;
		for (i = 0; i < file_list.length; i++) {
			arrFileInfo.push(file_list[i]);
		}
		for (j = 0; j < category_list.length; j++) {
			arrCategory.push(category_list[j]);
		}
	}
	...
	<form id="prodForm" name="prodForm" method="post">
									...
									<table class="amb_form_table lineAll">
										...
										<tbody>
											...
											<tr>
												...
												<td>
													<input type="text" id="prod_name" name="prod_name" class="inp" style="width: 90%" placeholder='<spring:message code="label.product.prod.input.name"/>' maxlength="40" value="${productInfo.prod_name}" />
												</td>
											</tr>
											<tr>
												<th><spring:message code="label.product.prod.price"/><i class="bullet mandatory"></i></th>
												<td>
													<input type="text" id="prod_price" name="prod_price" class="inp numCheck" style="width: 90%" placeholder='<spring:message code="label.product.prod.input.numberonly"/>' value="${productInfo.prod_price}" />
				...
				<div class="g_column w_1_1">
					<div class="unitBox" style="min-height: 600px;">
						<table class="amb_form_table lineAll">
							...
							<tbody>
								<textarea class="inp contents" id="prod_detail_txt" target_id="prod_detail" style="width: 90%;" placeholder='<spring:message code="label.product.prod.valid.input.content"/>'>${productInfo.prod_detail}</textarea>
							</tbody>
						</table>
					</div>
				</div>
				...
				<div class="g_column w_1_1">
					<div class="unitBox" style="min-height: 600px;">
						<table class="amb_form_table lineAll">
							...
							<tbody>
								<textarea class="inp contents" id="prod_delivery_info_txt" target_id="prod_delivery_info" style="width: 90%;" placeholder='<spring:message code="label.product.prod.valid.input.content"/>'>${productInfo.prod_delivery_info}</textarea>
							</tbody>
						</table>
					</div>
				</div>
				...
				<div class="g_column w_1_1">
					<div class="unitBox" style="min-height: 600px;">
						<table class="amb_form_table lineAll">
							...
							<tbody>
								<textarea class="inp contents" id="prod_refund_info_txt" target_id="prod_refund_info" style="width: 90%;" placeholder='<spring:message code="label.product.prod.valid.input.content"/>'>${productInfo.prod_refund_info}</textarea>
							</tbody>
						</table>
					</div>
				</div>
		...
		<div class="rowBox mgT30">
			<div class="fr btnArea middle">
				<a href="javascript:modifyAction();" class="amb_btnstyle blue middle"><spring:message code="label.common.modify"/></a>
